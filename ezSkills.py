from tkinter import Entry, Tk, X, LEFT, RIGHT, Checkbutton, IntVar, LabelFrame, Label, Button, END, StringVar, W
import os
...
root.geometry("350x165+" + str(int(x)) + "+" + str(int(y)))
root.title("ezSkills")
...
frame_char_name.pack(fill=X, padx=2, pady=2)

entry_char_name = Entry(frame_char_name, textvariable=entry_var_char_name)
entry_char_name.pack(fill=X, padx=5, pady=5)

frame_toggles = LabelFrame(root)
frame_toggles.pack(fill=X, padx=2, pady=2)
...
check_target.pack(side=LEFT, padx=5, pady=5)

check_f1 = Checkbutton(frame_toggles, text="Use Food", variable=var_food, onvalue=1, offvalue=0,
                       command=get_checkbutton_state)
check_f1.pack(side=LEFT, padx=5, pady=5)

check_f2 = Checkbutton(frame_toggles, text="Use Healing Spell", variable=var_health, onvalue=1, offvalue=0,
                       command=get_checkbutton_state)
check_f2.pack(side=LEFT, padx=5, pady=5)

frame_hotkeys = LabelFrame(root)
frame_hotkeys.pack(fill=X, padx=2, pady=2)

label_switch_target_hotkey = Label(frame_hotkeys, text="Target Key:", width=9, anchor=W)
label_switch_target_hotkey.pack(side=LEFT, pady=5)

entry_switch_target_hotkey = Entry(frame_hotkeys, width=5, textvariable=entry_var_target)
entry_switch_target_hotkey.pack(side=LEFT, pady=5)

label_food_hotkey = Label(frame_hotkeys, text="Food Key:", width=8, anchor=W)
label_food_hotkey.pack(side=LEFT, pady=5)

entry_food_hotkey = Entry(frame_hotkeys, width=5, textvariable=entry_var_food)
entry_food_hotkey.pack(side=LEFT, pady=5)

label_health_hotkey = Label(frame_hotkeys, text="Healing Spell Key:", width=14, anchor=W)
label_health_hotkey.pack(side=LEFT, pady=5)

entry_health_hotkey = Entry(frame_hotkeys, width=5, textvariable=entry_var_heal)
entry_health_hotkey.pack(side=LEFT, pady=5)

button_update_values = Button(root, text="Save", width=10, command=update_values)
button_update_values.pack(side=RIGHT, padx=2, pady=2)

label_save_config = Label(root, text="Configuration not saved.")
label_save_config.pack(side=LEFT, padx=2, pady=2)
